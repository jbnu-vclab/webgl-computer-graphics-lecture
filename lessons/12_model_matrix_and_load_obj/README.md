...
3. [`_shaders/basicFragment.js`] Basic Fragment Shader 구현
    ...
    중간에 `in vec2 v_texcoord`, `in vec3 v_normal`이 정점 셰이더에서 보간되어 넘어온 varying 값을 담고 있습니다. 현재는 이 값들을 사용하지는 않고, 그냥 빨간색을 하드코딩하여 출력하고 있습니다.
...
3. Fragment Shader에서 우리가 사용할 수 있는 변수들로 `v_normal`, `v_texcoord`가 있습니다. `v_normal`값을 사용해서 주전자 또는 육면체의 색상을 표현해 보세요. 그리고 왜 그러한 색상으로 표시되는 것인지 이유를 설명해 보세요.
